-- Phase 67: Publish and Copy Authority
-- 1. rpc_publish_rota_week: support p_allow_empty boolean default false to explicitly publish an empty week
-- 2. rpc_copy_previous_rota_week: source from latest published snapshot and published shifts, refusing if no published snapshot exists

-- Allow empty published snapshots with zero shifts (§2.2)
drop trigger if exists published_rota_snapshots_require_shifts on public.published_rota_snapshots;

drop function if exists public.rpc_publish_rota_week(uuid, uuid, boolean);
...
begin
  ...
  -- eligibility lock for every staff member assigned in this week.
  ...
  if draft_shift_count = 0 and not coalesce(p_allow_empty, false) then
  ...
  if published_shift_count = 0 and not coalesce(p_allow_empty, false) then
    raise exception 'cannot publish a rota week with no shifts' using errcode = '55000';
  end if;
  ...
  perform public.rpc_internal_write_audit(
    ...
    jsonb_build_object(
      ...
      'allow_empty', coalesce(p_allow_empty, false)
    )
  );
...
revoke all on function public.rpc_publish_rota_week(uuid, uuid, boolean, boolean) from public, anon;
grant execute on function public.rpc_publish_rota_week(uuid, uuid, boolean, boolean) to authenticated;

comment on function public.rpc_publish_rota_week(uuid, uuid, boolean, boolean) is
  'Manager-only atomic publish: locks assigned staff, derives approved one-off/recurring, leave and overlapping-shift scheduling clashes, requires explicit warning acknowledgement, then commits versioned snapshot + requests + targeted notifications + exact audit evidence atomically. Fourth parameter p_allow_empty defaults false; when true, permits publishing an empty week with 0 shifts.';

-- --------------------------------------------------------------------------
-- 2. rpc_copy_previous_rota_week: copy from latest published snapshot
-- --------------------------------------------------------------------------
...
declare
  ...
  latest_source_snapshot_id uuid;
...
begin
  ...
  -- Phase 67: Source from published truth only (§2.11).
  -- Distinct refusal when no published snapshot for the previous week exists.
  select snapshot.id into latest_source_snapshot_id
  from public.published_rota_snapshots as snapshot
  where snapshot.workspace_id = p_workspace_id
    and snapshot.rota_week_id = source_week_id
  order by snapshot.version desc
  limit 1;

  if latest_source_snapshot_id is null then
    raise exception 'No published rota found for the previous week to copy.' using errcode = '55000';
  end if;

  select count(*) into source_shift_count
  from public.published_rota_shifts as shift
  where shift.workspace_id = p_workspace_id
    and shift.snapshot_id = latest_source_snapshot_id;
  ...
  for src in
    select shift.department_id, shift.staff_member_id, shift.shift_date,
           ...
           src_shift.colour_override, src_shift.dept_override
    from public.published_rota_shifts as shift
    left join public.shifts as src_shift
      on src_shift.workspace_id = shift.workspace_id
     and src_shift.id = shift.source_shift_id
    where shift.workspace_id = p_workspace_id
      and shift.snapshot_id = latest_source_snapshot_id
  ...
  perform public.rpc_internal_write_audit(
    ...
    jsonb_build_object(
      ...
      'source_snapshot_id', latest_source_snapshot_id
    )
  );
...
comment on function public.rpc_copy_previous_rota_week(uuid, uuid, date) is
  'Transactional copy of the previous week into the target draft. Sources exclusively from the latest published snapshot of the previous week (§2.11). Reconstructs wall-clock timestamps in the location timezone, preserves overnight spans, replaces the target draft atomically, and marks the target week draft.';
