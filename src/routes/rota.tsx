...
const staff: { name: string; role: string; hrs: string; img: number; tone: string; shifts: Shift[] }[] = [
  { name: "Sophie Carter", role: "Manager", hrs: "32h", img: 5, tone: "info",
    shifts: [
  ...
  { name: "Daniel Mitchell", role: "Supervisor", hrs: "35h", img: 12, tone: "info",
    shifts: [
  ...
  { name: "Priya Patel", role: "Head Chef", hrs: "40h", img: 47, tone: "warning",
    shifts: [
  ...
  { name: "Liam O'Connor", role: "Bartender", hrs: "25h", img: 13, tone: "warning",
    shifts: [
  ...
  { name: "Olivia Bennett", role: "Barista", hrs: "20h", img: 16, tone: "info",
    shifts: [
  ...
  { name: "James Walker", role: "Waiting Staff", hrs: "20h", img: 14, tone: "purple",
    shifts: [
  ...
  { name: "Amelia Stone", role: "Housekeeping", hrs: "30h", img: 23, tone: "danger",
    shifts: [
  ...
  { name: "Noah Evans", role: "Porter", hrs: "15h", img: 33, tone: "success",
    shifts: [
...
function ShiftCell({ s }: { s: Shift }) {
  if (s.flag === "off") return <div className="h-16 flex items-center justify-center text-sm text-muted-foreground">— Day off</div>;
  if (s.flag === "open") {
    return (
      <div className={`h-16 rounded-lg border-2 ${toneStyles.open} flex flex-col items-center justify-center text-xs`}>
        <div className="font-medium">Open shift</div>
        <div className="flex items-center gap-1 text-[11px] text-muted-foreground">{s.role} <Plus className="h-3 w-3" /></div>
      </div>
    );
  }
  return (
    <div className={`h-16 rounded-lg border ${toneStyles[s.tone]} px-2.5 py-1.5 flex flex-col justify-between relative`}>
      <div className="text-xs font-semibold">{s.time}</div>
      <div className="text-[11px] text-muted-foreground">{s.role}</div>
      {s.flag === "conflict" && <AlertTriangle className="h-3.5 w-3.5 text-warning absolute top-1.5 right-1.5" />}
    </div>
  );
}
      ...
      <div className="grid grid-cols-12 gap-5">
        <Card className="col-span-12 lg:col-span-9 p-4">
          ...
          <div className="flex items-center gap-3 px-2 pb-3 border-b border-border">
            <button className="rounded-lg border border-border px-3 py-1.5 text-xs flex items-center gap-2"><Filter className="h-3.5 w-3.5" /> Filters</button>
            <span className="flex items-center gap-1.5 text-xs"><span className="rounded-md bg-warning-soft text-warning px-1.5 py-0.5 font-bold">2</span> Conflicts</span>
            <span className="flex items-center gap-1.5 text-xs"><span className="rounded-md bg-info-soft text-info px-1.5 py-0.5 font-bold">3</span> Open shifts</span>
            <span className="flex items-center gap-1.5 text-xs"><span className="rounded-md bg-accent-purple-soft text-accent-purple px-1.5 py-0.5 font-bold">1</span> Leave</span>
            <div className="ml-auto flex items-center gap-2">
              <button className="rounded-lg border border-border px-3 py-1.5 text-xs flex items-center gap-2"><Sparkles className="h-3.5 w-3.5 text-brand" /> Auto-fill</button>
              <button className="rounded-lg border border-border px-3 py-1.5 text-xs flex items-center gap-2"><SlidersHorizontal className="h-3.5 w-3.5" /> Templates</button>
            </div>
          </div>
              ...
              <thead>
                <tr className="text-left">
                  <th className="w-56 px-2">
                    <div className="text-sm font-semibold">Staff <span className="text-muted-foreground font-normal">(24)</span></div>
                    <div className="mt-2 flex items-center gap-2 rounded-lg border border-border px-2 py-1.5">
                      <Search className="h-3.5 w-3.5 text-muted-foreground" />
                      <input className="bg-transparent text-xs outline-none w-full" placeholder="Search staff..." />
                    </div>
                  </th>
                  {days.map((d) => (
                    <th key={d.d} className="px-1 align-top">
                      ...
                      <div className={`text-[11px] mt-0.5 ${d.tone === "danger" ? "text-danger" : d.tone === "warning" ? "text-warning" : "text-muted-foreground"}`}>⊘ {d.c}</div>
                    </th>
                  ))}
                </tr>
              </thead>
                  ...
                  <tr key={s.name}>
                    <td className="px-2">
                      <div className="flex items-center gap-2.5">
                        <img src={`https://i.pravatar.cc/64?img=${s.img}`} alt="" className="h-9 w-9 rounded-full object-cover" />
                        <div className="min-w-0">
                          <div className="text-sm font-medium truncate">{s.name}</div>
                          <div className="text-[11px] text-muted-foreground">{s.role}  ·  {s.hrs}</div>
                          <div className="text-[10px] text-muted-foreground">Contracted</div>
                        </div>
                      </div>
                    </td>
                    {s.shifts.map((sh, i) => (
                      <td key={i} className="px-1"><ShiftCell s={sh} /></td>
                    ))}
                  </tr>
                ))}
                <tr><td className="px-2 pt-2"><button className="w-full rounded-lg border border-dashed border-border py-2 text-xs flex items-center justify-center gap-1 text-muted-foreground"><Plus className="h-3.5 w-3.5" /> Add staff</button></td></tr>
              </tbody>
            </table>
          </div>
          ...
          <div className="px-2 pt-3 mt-1 border-t border-border flex flex-wrap items-center gap-4 text-[11px] text-muted-foreground">
            ...
            <div className="ml-auto flex items-center gap-3">
              ...
              <span className="flex items-center gap-1"><AlertTriangle className="h-3 w-3 text-warning" /> Conflict</span>
        ...
        <div className="col-span-12 lg:col-span-3 space-y-4">
          <Card className="p-5">
            ...
            <div className="mt-1 h-2 rounded-full bg-muted overflow-hidden"><div className="h-full w-[98%] bg-brand" /></div>
          ...
          <Card className="p-5">
            ...
            {[
              ...
              <div key={a.t} className="flex items-center gap-3 py-2 border-t first:border-t-0 border-border">
                <div className={`h-8 w-8 rounded-lg flex items-center justify-center ${a.tone === "danger" ? "bg-danger-soft text-danger" : "bg-warning-soft text-warning"}`}><a.icon className="h-4 w-4" /></div>
                <div className="flex-1"><div className="text-sm font-medium">{a.t}</div><div className="text-xs text-muted-foreground">{a.s}</div></div>
          ...
          <Card className="p-5">
            ...
            {[
              ...
              <div key={k} className="flex items-center justify-between py-1.5 text-sm">
                <span className="flex items-center gap-2 text-foreground"><span className="text-success">✓</span>{k}</span>
                <span className="text-muted-foreground">{v}</span>
              </div>
            ))}
            <button className="mt-4 w-full rounded-xl bg-brand text-brand-foreground py-2.5 text-sm font-semibold flex items-center justify-center gap-2"><Plane className="h-4 w-4" /> Ready to publish</button>
            <button className="mt-2 w-full rounded-xl border border-border py-2.5 text-sm font-medium flex items-center justify-center gap-2"><Share2 className="h-4 w-4" /> Share draft</button>
          </Card>
          ...
          <Card className="p-5">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              {[
                ["Management", "info"], ["Kitchen", "warning"],
                ["Bar", "danger"], ["Front of House", "info"],
                ["Housekeeping", "danger"], ["Porter", "success"],
              ].map(([t, tone]) => (
                <div key={t} className="flex items-center gap-2">
                  <span className={`h-2 w-2 rounded-full bg-${tone}`} style={{ background: `var(--${tone})` }} />
                  {t}
                </div>
              ))}
