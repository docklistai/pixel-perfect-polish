...
const items = [
  ...
  { Icon: FileText, label: "Approved hours export, not payroll" },
...
export function LandingTrustStrip() {
  return (
    <section className="border-b border-[#0c1412]/10 bg-[var(--landing-paper)] text-[var(--landing-ink)]">
      <ul className="mx-auto grid max-w-7xl grid-cols-1 divide-y divide-[#0c1412]/8 sm:grid-cols-2 sm:divide-x lg:grid-cols-3">
        {items.map((it) => (
          <li
            key={it.label}
