...
import { visibleSettingsTabs, SETTINGS_GROUPS } from "../data/settingsTabs";
...
export function SettingsSidebar({ activeTab, onTabChange }: SettingsSidebarProps) {
  const settingsTabs = visibleSettingsTabs(isPilotSurface());
  return (
    <>
      ...
      <Card className="hidden lg:block rounded-3xl p-2.5 self-start lg:sticky lg:top-6">
        <nav aria-label="Settings navigation">
          <div className="space-y-3">
            {SETTINGS_GROUPS.filter((group) => settingsTabs.some((t) => t.group === group.key)).map(
              (group) => {
